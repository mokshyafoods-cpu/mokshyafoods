...
export default function DashboardHero({ title, subtitle, revenue, orders, customers, products, lowStock }: any) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[1.75rem] border border-slate-700/70 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.22),_transparent_30%),linear-gradient(135deg,_#020617_0%,_#0f172a_45%,_#111827_100%)] p-4 text-slate-50 shadow-[0_24px_90px_rgba(2,6,23,0.45)] sm:p-5 lg:p-6">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-sky-400/25 bg-sky-400/10 px-3 py-1.5 text-xs font-medium text-sky-100 backdrop-blur sm:text-sm">
...
function MetricPill({ label, value, icon: Icon }: { label: string; value: string | number; icon: React.ElementType }) {
  return (
    <div className="rounded-[1.25rem] border border-slate-700/70 bg-slate-950/45 p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] backdrop-blur">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[9px] font-medium uppercase tracking-[0.25em] text-slate-400">{label}</p>
        <div className="rounded-lg border border-sky-400/20 bg-sky-400/10 p-1.5">
